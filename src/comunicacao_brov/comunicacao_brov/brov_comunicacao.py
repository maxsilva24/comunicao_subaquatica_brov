...
class CalcularVelocidadeSomAgua():       
    ...
    def calcular_equacao_NPL(self, temperatura, salinidade, profundidade, latitude):        
        """
        ...
        #
        if not (salinidade >= 0 and salinidade <= 42):
            raise Exception(
        ...
        Z=0
        resultado_equacao = 1402.5 + 5 * temperatura - 5.44 * pow(10,-2) * temperatura + 2.1 * pow(10,-4) * pow(temperatura,3) \
                            + 1.33 * salinidade - 1.23 * pow(10,-2) * salinidade * temperatura + 8.7 * pow(10,-5)*salinidade* pow(temperatura,2) \
                            + 1.56 * pow(10,-2) * Z + 2.55 * pow(10,-7) * pow(Z,2) - 7.3 * pow(10,-12)* pow(Z,3) \
                            + 1.2 * pow(10,-6) * Z * (latitude - 45) - 9.5 * pow(10,-13) * temperatura* pow(Z,3) \
                            + 3 * pow(10,-7) * pow(temperatura,2) * Z + 1.43 * pow(10,-5) * salinidade * Z
    ...
    def calcular_delay_transmissao(self,coordenadada_origem, coordenadada_destino,
                                        ...
                                        pressao     = None,profundidade=None, latitude=None):
        #
        tipoequacao  = tipoequacao  or self.tipoequacao  # isso é tipo um null coalescing do python, se esquerda fo nula recebe a direita
